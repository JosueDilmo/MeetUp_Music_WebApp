...
function DisplayEvents() {
  ...
  const [displayUserNames, setDisplayUserNames] = useState<usersFromDB[]>([]);
  ...
  const userNames = async (userId: string) => {
    // if (arrayOfJoined == null) {
    //   return "No artists joined yet";
    // } else {
    //   arrayOfJoined.map(async (userId: string) => {
    await axios(`http://localhost:3333/user-name/${userId}`)
      .then((response) => {
        setDisplayUserNames(response.data);
      })
      .catch((error) => {
        console.log(error);
      });
      ...
      {eventLoading ? (
        ...
        events.map((event) => (
          <div key={event.eventId}>
            <div className="break-inside relative overflow-auto flex flex-col justify-between space-y-3 text-sm rounded-xl max-w-[23rem] p-4 mb-4 bg-slate-800 text-white">
              ...
              <div>display users in the event</div>
              <div> WHEN STARTED -- WHEN END</div>
